...
class Cli {
    ...
    public static function shellEnv(): ShellEnv {
        ...
        elseif (PHP_SAPI === 'cli') return ShellEnv::NonInteractive;
    ...
    /**
     * True if running on the PHP built-in server.
     *
     * @since 0.11.2
     *
     * @return bool
    ...
    /**
     * Does the same as `choose`, but always asks yes/no and returns a boolean
     *
     * @see choose()
     *
     * @param string $question The question to ask the user.
    ...
    public static function confirm(string $question, bool $default = false): bool {
        $result = static::choose($question, 'yn', $default ? 'y' : 'n');
     ...
     * available, it falls back to basic strlen.
     *
    ...
    public static function safeStrlen(string $str, bool|string $encoding = false): int {
        ...
        if ((!$encoding || 'UTF-8' === $encoding) && static::canUsePcreX() && false !== ($length = preg_match_all('/\X/u', $str))) {
        ...
        if (function_exists('mb_strlen') && ($encoding || function_exists('mb_detect_encoding'))) {
            ...
            if ('UTF-8' === $encoding) {
                ...
                $length -= preg_match_all(static::getUnicodeRegexes()[1], $str);
            }
     ...
     * available, it falls back to substr().
     *
     * @param string        $str      The input string.
     * @param int           $start    The starting position of the substring.
     * @param null|int|bool $length   Optional, unless $is_width is set. Maximum length of the substring. Default false. Negative not supported.
    ...
    public static function safeSubstr(string $str, int $start, bool|int|null $length = false, bool|int $is_width = false, bool|string $encoding = false): bool|string {
        // Negative $length or $is_width and $length not specified aren't supported.
        ...
        // Normalise `$length` when not specified - PHP 5.3 substr takes false as full length, PHP > 5.3 takes null.
        ...
        if ((!$encoding || 'UTF-8' === $encoding) && static::canUsePcreX() && false !== ($try = preg_split('/(\X)/u', $str, $safe_strlen + 1,
                PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY)) && !preg_last_error()) {
    ...
    /**
     * Safely extracts a substring from the given string considering East Asian Width characters.
     *
     * @param string $str    The input string from which to extract the substring.
     * @param int    $length The maximum length of the extracted substring in terms of characters.
     *
     * @return string The extracted substring.
     */
    public static function _safeSubstrEaw(string $str, int $length): string {
        // Set the East Asian Width regex.
        $eaw_regex = static::getUnicodeRegexes()[0];
        ...
        if (preg_match($eaw_regex, $str)) {
            ...
            // See if it's all EAW.
            if (function_exists('mb_substr') && preg_match_all($eaw_regex, $str) === $length) {
    ...
    /**
     * Get width of string, i.e. length in characters, taking into account multibyte and mark characters for UTF-8, and multibyte for non-UTF-8.
     *
    ...
    public static function strWidth(string $string, string|bool $encoding = false): int {
        // Set the East Asian Width and Mark regexes.
        [
            $eaw_regex,
            $m_regex,
        ] = static::getUnicodeRegexes();
        ...
        // Assume UTF-8 if no encoding given - `preg_match_all()` will return false if given non-UTF-8 string.
        if ((!$encoding || 'UTF-8' === $encoding) && static::canUsePcreX() && false !== ($width = preg_match_all('/\X/u', $string))) {
    ...
    /**
     * Retrieves predefined Unicode regular expressions for East Asian Width and Mark characters.
     *
     * This method returns an array containing two regular expression patterns:
     * 1. A regex pattern for East Asian Width (EAW) characters, which are considered as wide/fullwidth.
     * 2. A regex pattern for Mark characters, which include combining marks ("Mc"), enclosing marks ("Me"),
     *    and non-spacing marks ("Mn") that should be ignored for spacing purposes.
     *
     * These patterns are loaded from a predefined file generated from Unicode data.
     *
     * @return array An array where the first element is the East Asian Width regex pattern and
     *               the second element is the Mark characters' regex pattern.
     */
    public static function getUnicodeRegexes(): array {
        ...
        // Load both regexes generated from Unicode data.
